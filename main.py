...
import time
from utils import *
from torch.optim.lr_scheduler import ReduceLROnPlateau, LinearLR, MultiStepLR
...
train_indices, val_indices = train_test_split(list(range(len(train_labels))),
                                              test_size=0.125, stratify=train_labels, random_state=args.seed,
                                              shuffle=True)
...
for index in range(args.runs):
    ...
    # gnn = eval(args.model)
    # model = ResidualGNNs(args, train_dataset, args.hidden, args.hidden_mlp, args.num_layers, gnn).to(
    #     args.device)  ## apply GNN*
    model = GCN(args, train_dataset, args.hidden, args.hidden_mlp, args.num_layers).to(args.device)  ## apply GNN*
    print(model)
    total_params = sum(p.numel() for p in model.parameters())
    print(f"Total number of parameters is: {total_params}")
    optimizer = Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    # scheduler = ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=5, verbose=True)
    # scheduler = LinearLR(optimizer, start_factor=0.99, total_iters=100)
    ...
    for epoch in range(args.epochs):
        ...
        # if epoch%10==0:
        print(
            "epoch: {}, loss: {}, val_acc:{}, test_acc:{}".format(epoch, np.round(loss.item(), 6), np.round(val_acc, 6),
                                                                  np.round(test_acc, 6)))
